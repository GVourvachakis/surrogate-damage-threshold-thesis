...
it is advanced by the explicit Heun part without degrading the practical stability of the scheme;
the corresponding stability discussion is deferred to \S\ref{sec:ch2-stability}.
...
\subsection{Well-posedness of the discrete \& continuous nonlinear system}
\label{sec:ch2-wellposedness}

The well-posedness argument for the nonlinear solver has a three-level hierarchy. At the continuous PDE level, one studies the quasilinear parabolic system \eqref{eq:ch2-dimensionless-pttm}. After spatial discretisation, one obtains the finite-dimensional ODE system \eqref{eq:ch2-abstract-ode}. Finally, at each timestep the stage solve is reduced to the frozen-coefficient\footnote{In this context, the term 'frozen-coefficient' refers to the practice of evaluating state-dependent coefficients at the fixed values provided by the current iteration guess \((\mathbf{u}^{(k)},\mathbf{v}^{(k)})\).} linear system \eqref{eq:ch2-picard-nonlinear-1d}. These three levels use distinct fixed-point arguments: Banach contraction for the discrete Picard map, Picard--Lindel\"of for the method-of-lines system, and quasilinear parabolic theory for the continuous model.

\begin{proposition}[Contraction of the Picard map]
    \label{prop:ch2-picard-contraction}
    Let \(\Delta_t>0\) and suppose that the nonlinear coefficients \(D_1\), \(c_1\), and \(c_2\) are uniformly Lipschitz continuous on the temperature range \([u_{\min},u_{\max}]\) with Lipschitz constant \(L\). Let \(\mathcal{F}_k:\mathbb{R}^{2N}\to\mathbb{R}^{2N}\) denote the Picard map induced by the frozen-coefficient linear solve \eqref{eq:ch2-picard-nonlinear-1d}. Then there exist constants \(C,C'>0\), depending only on the physical parameters and the discrete operators, such that
    \[
        \|\mathcal{F}_k[\mathbf{x}] - \mathcal{F}_k[\mathbf{y}]\|_2
        \le
        \kappa
        ...
        \kappa
        \coloneqq
        \frac{CL\Delta_t}{1 + C'\Delta_t^{-1}}.
    \]
    For sufficiently small \(\Delta_t\), one has \(\kappa<1\), so \(\mathcal{F}_k\) is a contraction on a closed ball \(B_r \subset \mathbb{R}^{2N}\). Consequently, Algorithm~\ref{alg:ch2-picard} converges linearly to the unique fixed point \(\mathbf{x}^{n+1}_{*}\).
\end{proposition}
\begin{proof}[Proof sketch]
    Subtracting the two frozen-coefficient systems associated with \(\mathbf{x}\) and \(\mathbf{y}\) shows that the difference \(\mathcal{F}_k[\mathbf{x}] - \mathcal{F}_k[\mathbf{y}]\) is controlled by the difference of the right-hand sides. The Lipschitz continuity of \(D_1\), \(c_1\), and \(c_2\) implies that this right-hand-side difference is \(\mathcal{O}(L\Delta_t\|\mathbf{x}-\mathbf{y}\|_2)\), while the inverse of the block matrix in \eqref{eq:ch2-picard-nonlinear-1d} is bounded uniformly on \(B_r\) by discrete ellipticity. Hence the Picard map is a contraction for sufficiently small \(\Delta_t\), and the Banach fixed-point theorem yields existence, uniqueness, and linear convergence of the iteration. The acceleration step preserves the convergent fixed point and improves the practical rate, as discussed by \cite{Pollock_2018}.
\end{proof}

\refstepcounter{theorem}
\paragraph{Remark \thetheorem\ (Picard--Lindel\"of for the method-of-lines system).}\label{rem:ch2-picard-lindelof}
After spatial discretisation, the pTTM reduces to the ODE system \eqref{eq:ch2-abstract-ode}. Under the regularity assumptions encoded in the constitutive closures \eqref{eq:ch2-dimensionless-coeffs}, the right-hand sides \(\mathbf{r}^{1}\) and \(\mathbf{r}^{2}\) are Lipschitz continuous in \((\mathbf{u},\mathbf{v})\) uniformly in \(t\). The Picard--Lindel\"of theorem, in the form of \cite[Theorem~2.2]{Teschl_2012}, therefore guarantees local existence and uniqueness on \([0,\tau^{*}]\) for some \(\tau^{*}>0\). Since the present study remains in the sub-ablation regime and the coefficients remain bounded on the relevant temperature interval, the solution extends by standard continuation arguments.

\refstepcounter{theorem}
\paragraph{Remark \thetheorem\ (Local well-posedness of the continuous pTTM).}
\label{rem:ch2-pde-wellposedness}
Under the assumptions that the constitutive closures in \eqref{eq:ch2-dimensionless-coeffs} are sufficiently smooth on a temperature rectangle (\(\mathbb{R}_+ \equiv (0, +\infty)\))
\[
    \mathcal R=[m_u,M_u]\times[m_v,M_v] \subset \mathbb{R}_+^2,
\]
that the effective diffusion coefficient
\[
    a(u,v)\coloneqq D_1(u)K(u,v)
\]
satisfies
\[
    0<\lambda\le a(u,v)\le\Lambda,
    \qquad
    0<D_2\le\Lambda
    \qquad\text{for all }(u,v)\in\mathcal R,
\]
and that \(C_e(T_{\mathrm{ref}}u)\ge c_*>0\) on \([m_u,M_u]\), the system \eqref{eq:ch2-dimensionless-pttm} is uniformly quasilinear parabolic. For compatible initial and boundary data of Hölder class, standard results then imply local existence and uniqueness of a classical solution
\[
    (u,v)\in C^{2+\alpha,\,1+\alpha/2}(\overline{\Omega}\times[0,\tau^*])
\]
for some \(\tau^*>0\); see \cite{Ladyzhenskaya_1968,Amann_1995}. A related fixed-point interpretation for nonlinear parabolic problems may be found in \cite[Chapter~15]{Taylor_2023}. In the present physical setting, this conclusion is therefore valid only on temperature intervals bounded away from \(u=0\), since the coefficients \(D_1\), \(c_1\), and \(s\) inherit the factor \(1/C_e(T_{\mathrm{ref}}u)\).

The convergence of the Picard iteration established in Proposition~\ref{prop:ch2-picard-contraction} is a local, per-timestep result.
The cumulative error propagation over many timesteps is controlled by the stability and convergence analysis of \S\ref{sec:ch2-stability}.

\refstepcounter{theorem}
\paragraph{Remark \thetheorem\ (Energy estimate).}\label{rem:ch2-rectangular-energy}
The continuous well-posedness discussion above concerns the dimensionless rectangular system \eqref{eq:ch2-dimensionless-pttm}. Appendix~\ref{app:ch2-rectangular-energy} records a complementary energy estimate for the corresponding coefficient-fixed rectangular problem, using the standard spaces \(H=L^2(\Omega)\) and \(V=H^1_0(\Omega)\) after the usual lifting of inhomogeneous Dirichlet data. The appendix uses fixed coefficients \(a_e,c_{1,*},c_{2,*}\) and prescribed source forcing \(s_*\), so it provides a priori control and continuous dependence for the continuous PDE setting. A compact set of linear Au numerical consistency diagnostics is reported in \S\ref{sec:app-ch2-energy-diagnostics}.

\refstepcounter{theorem}
\paragraph{Theorem \thetheorem\ (Continuous coefficient-fixed energy estimate).}\label{thm:ch2-coeff-fixed-energy}
Let \(H=L^2(\Omega)\) and \(V=H^1_0(\Omega)\), after lifting any inhomogeneous Dirichlet data. If \(0<\lambda_e\le a_e\le\Lambda_e\), \(0\le c_{1,*},c_{2,*}\le C_c\), \(D_2>0\), and \(s_*\in L^2(0,\tau_f;V')\), then any weak solution of the coefficient-fixed rectangular problem satisfies
\[
    \begin{aligned}
         & \operatorname*{ess\,sup}_{0\le t\le\tau_f}
        \left(\|u(t)\|_H^2+\|v(t)\|_H^2\right)
        +
        \int_0^{\tau_f}
        \left(\|\nabla u(t)\|_H^2+\|\nabla v(t)\|_H^2\right)\,dt
        \\
         & \qquad
        \le
        C\left(
        \|u_0\|_H^2+\|v_0\|_H^2+\|s_*\|_{L^2(0,\tau_f;V')}^2
        \right),
    \end{aligned}
\]
where \(C\) depends on the coefficient bounds, \(\tau_f\), and \(\Omega\), but not on a discretisation parameter. The proof is given in Appendix~\ref{app:ch2-rectangular-energy}; this is continuous PDE theory complementary to, not a replacement for, the discrete stability analysis of \S\ref{sec:ch2-stability}.
...
Although we have established the fixed-point structure of the discrete and semi-discrete formulations,
it is useful to record the corresponding local existence result for the continuous pTTM that underlies the threshold simulations.
\begin{proposition}[Local existence and uniqueness for the quasilinear pTTM]
    \label{prop:ch2-existence}
    Let \(\Omega=(a,b)\subset\mathbb R\) be a bounded interval, let \(0<\alpha<1\), and consider the system \eqref{eq:ch2-dimensionless-pttm} supplemented with initial data
    \[
        u(\cdot,0)=u_0,\qquad v(\cdot,0)=v_0 \quad \text{in }\Omega,
    \]
    and Dirichlet boundary data prescribed on \(\partial\Omega\times[0,\tau_f]\). Assume that:
    \begin{enumerate}
        \item the initial data satisfy
              \[
                  u_0,v_0 \in C^{2+\alpha}(\overline{\Omega}),
              \]
              and the boundary data belong to \(C^{2+\alpha,\,1+\alpha/2}(\partial\Omega\times[0,\tau_f])\), with the usual compatibility conditions at \(t=0\);

        \item there exists a closed interval rectangle
              \[
                  \mathcal R=[m_u,M_u]\times[m_v,M_v] \subset \mathbb{R}_+^2
              \]
              containing the range of the initial and boundary data, such that the constitutive coefficients
              \[
                  D_1,\quad K,\quad c_1,\quad c_2,\quad s
              \]
              are sufficiently smooth on \(\mathcal R\), with \(D_1,c_1,c_2\in C^{1+\alpha}\) in \(u\), \(K\in C^{1+\alpha}\) in \((u,v)\), and \(s\) Hölder continuous in \((t,x)\) and locally \(C^{1+\alpha}\) in \((u,v)\);

        \item the electron heat capacity is non-degenerate on the relevant temperature range, i.e.
              \[
                  C_e(T_{\mathrm{ref}}u)\ge c_*>0
                  \qquad \text{for all }u\in[m_u,M_u];
              \]

        \item the principal diffusion coefficient
              \[
                  a(u,v)\coloneqq D_1(u)K(u,v)
              \]
              is uniformly positive and bounded on \(\mathcal R\), namely there exist constants \(0<\lambda\le\Lambda<\infty\) such that
              \[
                  \lambda \le a(u,v)\le \Lambda
                  \qquad \text{for all }(u,v)\in\mathcal R,
              \]
              and \(D_2>0\).
    \end{enumerate}
    Then there exists \(\tau^*>0\) such that the dimensionless pTTM admits a unique classical solution
    \[
        (u,v)\in C^{2+\alpha,\,1+\alpha/2}\bigl(\overline{\Omega}\times[0,\tau^*]\bigr)
    \]
    satisfying the prescribed initial and boundary data. Moreover, the solution depends continuously on the initial and boundary data in the corresponding Hölder norms.
\end{proposition}
\begin{proof}[Sketch of justification]
    ...
    \[
        a(u,v)\coloneqq D_1(u)K(u,v).
    \]
    Then the first equation in \eqref{eq:ch2-dimensionless-pttm} can be expanded as
    \[
        u_t
        =
        a(u,v)u_{xx}
        +
        D_1(u)\,\partial_x K(u,v)\,\partial_x u
        -
        c_1(u)(u-v)
        +
        s(t,x;u,v),
    ...
    \[
        v_t = D_2 v_{xx} + c_2(u)(u-v).
    \]
    Hence the system is quasilinear parabolic, with diagonal principal part
    \[
        \operatorname{diag}(a(u,v),D_2),
    \]
    and is uniformly parabolic under the assumptions above. The stated regularity and compatibility conditions on the data allow one to apply the standard local existence theory for quasilinear parabolic systems in Hölder spaces; see, for example, \cite{Ladyzhenskaya_1968,Amann_1995}. This yields a time \(\tau^*>0\) and a unique solution
    \[
        (u,v)\in C^{2+\alpha,\,1+\alpha/2}(\overline{\Omega}\times[0,\tau^*]).
    \]
    Continuous dependence on the data follows from the same theory.

    The semi-discrete fixed-point and Picard--Lindel\"of arguments established earlier are consistent with this continuous result, but they do not replace the PDE-level quasilinear parabolic existence theorem.
\end{proof}
...
The discretisations developed in \S\S\ref{sec:ch2-linear}--\ref{sec:ch2-nonlinear} are second-order accurate in both time and space. For the linear constant-coefficient scheme, stability is analysed by a modal argument, yielding unconditional stability of the monolithic Crank--Nicolson discretisation. This should be distinguished from the approach of Zhai et al.\ \cite{Zhai_2020}, whose compact finite-difference scheme is analysed by a discrete-energy method. For the present centred second-order discretisation, the same conclusion is obtained through a Fourier, or von Neumann, argument. The nonlinear IMEX Heun--Crank--Nicolson scheme is then assessed through the IMEX stability-function framework of \cite{Ascher_1997}, together with the partitioned predictor results of \cite{Huang_2019}. Empirical confirmation is reported at the end of the section through dedicated time- and space-refinement studies.

\subsection{Von Neumann framework}
\label{sec:ch2-stability-setup}

The stability analysis is carried out for the constant-coefficient one-dimensional pTTM obtained from \eqref{eq:ch2-dimensionless-pttm} by freezing \(D_1\), \(D_2\), \(c_1\), and \(c_2\), and by omitting the source term. The resulting homogeneous system is
\begin{subequations}\label{eq:ch2-ode-constant-coeff}
    \begin{align}
        \partial_t u & = D_1 \partial_{xx} u - c_1(u-v), \\
        \partial_t v & = D_2 \partial_{xx} v + c_2(u-v).
    \end{align}
\end{subequations}
For a uniform grid \(x_j = j\Delta_x\), we substitute the Fourier modes
\[
    u_j^n = \xi_e^n e^{i\kappa j\Delta_x},
    \qquad
    v_j^n = \xi_l^n e^{i\kappa j\Delta_x},
    \qquad
    \kappa \in [0,\pi/\Delta_x].
\]
The centred second difference acting on \(e^{i\kappa j\Delta_x}\) produces the discrete Laplacian symbol
\begin{equation}
    \sigma(\kappa)
    =
    \frac{e^{i\kappa\Delta_x} - 2 + e^{-i\kappa\Delta_x}}{\Delta_x^2}
    =
    -\frac{4\sin^2(\kappa\Delta_x/2)}{\Delta_x^2}.
    \label{eq:ch2-fourier-symbol}
\end{equation}
Hence \(\sigma(\kappa) \in [-4/\Delta_x^2,0]\) for all admissible wavenumbers, \(\sigma(\kappa)=0\) at \(\kappa=0\), and \(|\sigma(\kappa)|\) attains its maximum at \(\kappa=\pi/\Delta_x\).

After the Fourier substitution, \eqref{eq:ch2-ode-constant-coeff} reduces to a two-dimensional linear system for the modal amplitudes \(\boldsymbol{\xi}^n \coloneqq (\xi_e^n,\xi_l^n)^\top\),
\[
    \partial_t \boldsymbol{\xi}
    =
    \mathbf{A}(\kappa)\boldsymbol{\xi},
\]
...
\begin{equation}
    ...
    =
    \begin{bmatrix}
        D_1\sigma(\kappa) - c_1 & c_1                     \\
        c_2                     & D_2\sigma(\kappa) - c_2
    \end{bmatrix}.
    \label{eq:ch2-matrix-A}
\end{equation}
This matrix is the central object in the von Neumann analysis because its spectral properties determine the amplification behaviour of every Fourier mode.

\begin{lemma}[Spectral properties of \(\mathbf{A}(\kappa)\)]
    \label{lem:ch2-spectrum-A}
    For all \(\kappa \in [0,\pi/\Delta_x]\) and all \(D_1,D_2,c_1,c_2>0\), the matrix \(\mathbf{A}(\kappa)\) satisfies:
    \begin{enumerate}
        \item \(\operatorname{tr}\!\bigl(\mathbf{A}(\kappa)\bigr) = (D_1+D_2)\sigma(\kappa) - (c_1+c_2) \le 0\);
        \item \(\det\!\bigl(\mathbf{A}(\kappa)\bigr) = D_1D_2\sigma(\kappa)^2 - (D_1c_2 + D_2c_1)\sigma(\kappa) \ge 0\);
        \item every eigenvalue \(\lambda\) of \(\mathbf{A}(\kappa)\) satisfies \(\operatorname{Re}(\lambda)\le 0\).
    \end{enumerate}
\end{lemma}
\begin{proof}
    The trace formula follows immediately from \eqref{eq:ch2-matrix-A}. Since \(\sigma(\kappa)\le 0\) and \(c_1,c_2>0\), one obtains \(\operatorname{tr}(\mathbf{A}(\kappa))\le 0\). For the determinant,
    \[
        \det\!\bigl(\mathbf{A}(\kappa)\bigr)
        =
        \bigl(D_1\sigma(\kappa)-c_1\bigr)\bigl(D_2\sigma(\kappa)-c_2\bigr) - c_1c_2
        =
        D_1D_2\sigma(\kappa)^2 - (D_1c_2 + D_2c_1)\sigma(\kappa),
    \]
    and this is non-negative because \(\sigma(\kappa)^2 \ge 0\) and \(-\sigma(\kappa)\ge 0\). Let \(\Delta \coloneqq \operatorname{tr}(\mathbf{A}(\kappa))^2 - 4\det(\mathbf{A}(\kappa))\). If \(\Delta \ge 0\), the eigenvalues are real and satisfy
    \[
        \lambda_{1,2}
        =
        \frac{1}{2}\left(\operatorname{tr}(\mathbf{A}(\kappa)) \pm \sqrt{\Delta}\right).
    \]
    Since \(\det(\mathbf{A}(\kappa))\ge 0\), one has \(\Delta \le \operatorname{tr}(\mathbf{A}(\kappa))^2\), hence \(\sqrt{\Delta}\le |\operatorname{tr}(\mathbf{A}(\kappa))|\). Therefore \(\lambda_{1,2}\le \frac{1}{2}\bigl(\operatorname{tr}(\mathbf{A}(\kappa)) + |\operatorname{tr}(\mathbf{A}(\kappa))|\bigr)\le 0\). If \(\Delta < 0\), the eigenvalues form a complex-conjugate pair and their real part is \(\operatorname{tr}(\mathbf{A}(\kappa))/2 \le 0\). In both cases \(\operatorname{Re}(\lambda)\le 0\).
\end{proof}

\subsection{Stability of the linear Crank--Nicolson scheme}
\label{sec:ch2-stability-linear}

Applying the Crank--Nicolson discretisation to \eqref{eq:ch2-ode-constant-coeff} and inserting the Fourier ansatz yields the single-mode update
\[
    \boldsymbol{\xi}^{n+1}
    =
    \mathbf{G}(\kappa)\boldsymbol{\xi}^{n},
    \qquad
    \boldsymbol{\xi}^{n}
    \coloneqq
    \begin{bmatrix}
...
\begin{equation}
    \mathbf{G}(\kappa)
    =
    \left(\mathbf{I} - \frac{\Delta_t}{2}\mathbf{A}(\kappa)\right)^{-1}
    \left(\mathbf{I} + \frac{\Delta_t}{2}\mathbf{A}(\kappa)\right).
    \label{eq:ch2-amplification-matrix}
\end{equation}
This is the matrix Cayley transform of \(\mathbf{A}(\kappa)\), analogous to the amplification construction used by \cite{Zhai_2020} in their compact finite-difference setting.

To expose the parameter dependence, we introduce the mesh Fourier numbers
\[
    \mu_e \coloneqq D_1 \sigma(\kappa)\Delta_t,
    \qquad
    \mu_l \coloneqq D_2 \sigma(\kappa)\Delta_t,
\]
and the dimensionless coupling numbers
\[
    \gamma_e \coloneqq c_1\Delta_t,
    \qquad
    \gamma_l \coloneqq c_2\Delta_t.
\]
Since \(\sigma(\kappa)\le 0\), one has \(\mu_e,\mu_l\le 0\), whereas \(\gamma_e,\gamma_l>0\). Therefore
\[
    \frac{\Delta_t}{2}\mathbf{A}(\kappa)
    =
    \frac{1}{2}
    \begin{bmatrix}
        \mu_e - \gamma_e & \gamma_e         \\
        \gamma_l         & \mu_l - \gamma_l
    \end{bmatrix}.
\]
Defining
\[
    \mathbf{P}
    \coloneqq
    \mathbf{I} - \frac{\Delta_t}{2}\mathbf{A}(\kappa),
    \qquad
    \mathbf{Q}
    \coloneqq
    \mathbf{I} + \frac{\Delta_t}{2}\mathbf{A}(\kappa),
...
\[
    ...
    =
    \begin{bmatrix}
        1 - (\mu_e-\gamma_e)/2 & -\gamma_e/2            \\
        -\gamma_l/2            & 1 - (\mu_l-\gamma_l)/2
    \end{bmatrix},
    \qquad
    \mathbf{Q}
    =
    ...
    \end{bmatrix}.
\]
Writing \(p_{ij}\) and \(q_{ij}\) for the entries of \(\mathbf{P}\) and \(\mathbf{Q}\), respectively, the amplification matrix is
\begin{equation}
    \mathbf{G}(\kappa)
    =
    \frac{1}{\det(\mathbf{P})}
    \begin{bmatrix}
        p_{22}q_{11} - p_{12}q_{21}  & p_{22}q_{12} - p_{12}q_{22}  \\
        -p_{21}q_{11} + p_{11}q_{21} & -p_{21}q_{12} + p_{11}q_{22}
    \end{bmatrix}.
    \label{eq:ch2-G-explicit}
\end{equation}
The explicit inversion is well defined because \(\det(\mathbf{P})>0\), as shown below.

\begin{theorem}[Unconditional modal stability of the linear CN--CN scheme]
    \label{thm:ch2-cn-stability}
    Let \(D_1,D_2>0\) and \(c_1,c_2>0\). For all \(\kappa \in [0,\pi/\Delta_x]\) and all \(\Delta_t>0\), \(\Delta_x>0\), the spectral radius of \(\mathbf{G}(\kappa)\) satisfies
    \[
        \operatorname{spr}\!\bigl(\mathbf{G}(\kappa)\bigr) \le 1.
    \]
    Consequently, the monolithic CN--CN scheme \eqref{eq:ch2-block-1d} for the linear pTTM is unconditionally stable in the modal, or von Neumann, sense for the constant-coefficient periodic problem. For the corresponding bounded-interval homogeneous Dirichlet problem, the same conclusion follows by replacing the Fourier basis with the discrete sine basis of the one-dimensional Laplacian.
\end{theorem}
\begin{proof}
    Let \(\lambda\) be an eigenvalue of \(\mathbf{A}(\kappa)\). By Lemma~\ref{lem:ch2-spectrum-A}, \(\operatorname{Re}(\lambda)\le 0\). If \(\mathbf{A}(\kappa)\mathbf{v}=\lambda \mathbf{v}\), then
    \[
        \mathbf{G}(\kappa)\mathbf{v}
        =
        \left(\mathbf{I} - \frac{\Delta_t}{2}\mathbf{A}(\kappa)\right)^{-1}
        \left(\mathbf{I} + \frac{\Delta_t}{2}\mathbf{A}(\kappa)\right)\mathbf{v}
        =
        \frac{1 + \frac{\Delta_t}{2}\lambda}{1 - \frac{\Delta_t}{2}\lambda}\mathbf{v},
    \]
    provided \(1-\frac{\Delta_t}{2}\lambda \neq 0\). This denominator never vanishes because \(\operatorname{Re}(\lambda)\le 0\), hence \(\operatorname{Re}(1-\frac{\Delta_t}{2}\lambda)\ge 1\), so \(\det(\mathbf{P})=\prod_j(1-\frac{\Delta_t}{2}\lambda_j)>0\). Thus every eigenvalue \(\mu\) of \(\mathbf{G}(\kappa)\) is of the form
    \[
        \mu
        =
        \frac{1 + \frac{\Delta_t}{2}\lambda}{1 - \frac{\Delta_t}{2}\lambda}.
    \]
    Writing \(\lambda = a+ib\) with \(a\le 0\), one obtains
    \[
        |\mu|^2
        =
        \frac{\left(1 + \frac{\Delta_t}{2}a\right)^2 + \left(\frac{\Delta_t}{2}b\right)^2}{\left(1 - \frac{\Delta_t}{2}a\right)^2 + \left(\frac{\Delta_t}{2}b\right)^2}.
    \]
    Since \(a\le 0\), the numerator does not exceed the denominator, and therefore \(|\mu|\le 1\). This holds for every eigenvalue of \(\mathbf{G}(\kappa)\), so the spectral radius of \(\mathbf{G}(\kappa)\) is bounded by \(1\) for all admissible wavenumbers and mesh parameters.
\end{proof}
The same argument extends to the two-dimensional block system \eqref{eq:ch2-block-2d}, because the two-dimensional Laplacian symbol is the sum of two one-dimensional symbols and the positivity of \(\det(\mathbf{P})\) is preserved.

\subsection{Stability of the IMEX Heun--Crank--Nicolson scheme}
\label{sec:ch2-stability-imex}

In the nonlinear scheme of \S\ref{sec:ch2-nonlinear}, the laser source
\(s(t,\mathbf{x};u,v)\) is evaluated at the current Picard iterate before the
linear stage solve begins. From the perspective of the Picard-linearised
system \eqref{eq:ch2-picard-nonlinear-1d}, the source is therefore a frozen,
explicitly treated known forcing contribution within that solve, while the stiff diffusion and coupling
terms are treated implicitly. The homogeneous stiff part is governed by the
same modal amplification mechanism as the coefficient-fixed CN--CN analysis
above.

The scalar IMEX stability-function calculation below is a supplementary
perturbative bound for the conservative case in which the explicit
source-dependent correction is interpreted as a small linearised contribution.
Represent the implicit and explicit pieces by the additive test equation
\(\dot{y}=\lambda_I y+\lambda_E y\), where \(\operatorname{Re}(\lambda_I)\le0\)
by Lemma~\ref{lem:ch2-spectrum-A}, and where \(\lambda_E\) denotes a bounded
linearised source contribution. Introducing
\[
    \hat{z} \coloneqq \Delta_t \lambda_E,
    \qquad
    z \coloneqq \Delta_t \lambda_I,
\]
the source non-stiffness assumption is precisely \(|\hat{z}| \ll 1\).

Following \cite[\S 3]{Ascher_1997}, the stability function of the two-stage IMEX pair applied to \(\dot{y} = \lambda_E y + \lambda_I y\) is
\[
    R(\hat{z},z)
    =
    1
    +
    \bigl(\hat{z}\hat{\mathbf{b}}^{\top} + z\mathbf{b}^{\top}\bigr)
    \bigl(\mathbf{I} - z\mathbf{A} - \hat{z}\hat{\mathbf{A}}\bigr)^{-1}\mathbf{1},
\]
with Butcher matrices
\[
    \hat{\mathbf{A}}
    =
    \begin{bmatrix}
...
For this pair,
\[
    \mathbf{I} - z\mathbf{A} - \hat{z}\hat{\mathbf{A}}
    =
    \begin{bmatrix}
        1              & 0     \\
        -(\hat{z}+z/2) & 1-z/2
    \end{bmatrix},
\]
so the inverse is obtained explicitly and yields
\begin{equation}
    R(\hat{z},z)
    =
    \frac{1 + \hat{z} + \frac{1}{2}\hat{z}^2 + \frac{1}{2}(1+\hat{z})z}{1-z/2}.
    \label{eq:ch2-imex-stability-fn}
\end{equation}
The implicit part is stiffly accurate because \(\mathbf{b}^{\top}\) coincides with the last row of \(\mathbf{A}\), so the last implicit stage directly determines the Crank--Nicolson contribution to the final update.

\begin{proposition}[Stability of the IMEX Heun--Crank--Nicolson scheme]
    \label{prop:ch2-imex-stability}
    Let \(z = \Delta_t \lambda_I\) satisfy \(\operatorname{Re}(z)\le 0\), and suppose that \(|\hat{z}| = |\Delta_t \lambda_E| \le \varepsilon\) for some \(\varepsilon>0\). Then there exists \(\varepsilon_0>0\), depending only on the physical parameters entering the source bound, such that for every \(\varepsilon \le \varepsilon_0\),
    \[
        |R(\hat{z},z)| \le 1 + C\varepsilon
    \]
    with a constant \(C>0\) independent of \(z\). Thus the scalar IMEX perturbation remains controlled when the explicit source-dependent contribution is non-stiff relative to the timestep.
\end{proposition}
\begin{proof}[Proof sketch]
    From \eqref{eq:ch2-imex-stability-fn}, write
    \[
        R(\hat{z},z)
        =
        R_0(z)
        ...
        R_1(\hat{z},z)
        \coloneqq
        \frac{1+\hat{z}/2+z/2}{1-z/2}.
    \]
    The factor \(R_0(z)\) is the Crank--Nicolson amplification factor, so \(|R_0(z)|\le 1\) whenever \(\operatorname{Re}(z)\le 0\), by Theorem~\ref{thm:ch2-cn-stability}. Moreover, \(|1-z/2|\ge 1\) for \(\operatorname{Re}(z)\le 0\), and also \(|z| \le 2|1-z/2|\), so \(R_1(\hat{z},z)\) is uniformly bounded on the closed left half-plane. Hence
    \[
        |R(\hat{z},z)|
        \le
        |R_0(z)| + |\hat{z}|\,|R_1(\hat{z},z)|
        \le
        1 + C|\hat{z}|
        \le
        1 + C\varepsilon.
    \]
    Choosing \(\varepsilon_0\) so that the source contribution remains in this perturbative regime gives the claim. The extension from the scalar test problem to the vector-valued partitioned system follows by applying the same bound to each modal eigenvalue, in agreement with \cite[\S 3]{Ascher_1997} and the partitioned stability result of \cite[Theorem~3.1]{Huang_2019}. This is a perturbative bound for the explicit contribution, not an unconditional nonlinear stability result.
\end{proof}

\begin{remark}
    For the gold benchmark of Table~\ref{tab:ch2-params}, with \(\Delta_t \sim 0.1\,\mathrm{ps}\) and source magnitude bounded by \(F^*/t_p\), one obtains \(|\hat{z}| \lesssim 10^{-3}\) in dimensionless form, well inside the perturbative regime of \eqref{eq:ch2-imex-stability-fn}. By contrast, the explicit-coupling IMEX variant discussed later in the convergence and runtime comparison has \(\hat{z}\) containing the coupling term \(c_1\Delta_t = \mathcal{O}(1)\), which violates the non-stiff explicit assumption and explains the degraded temporal accuracy observed for that split.
\end{remark}

\subsection{Convergence analysis}
\label{sec:ch2-convergence}

The convergence proof combines the consistency of the centred space--time discretisation with the modal stability established in \S\ref{sec:ch2-stability-linear}. For the linear constant-coefficient scheme, the relevant defects are obtained directly from Taylor expansion. The nonlinear IMEX scheme then inherits the same formal order provided the coefficient variations remain Lipschitz-continuous along the Picard iterates.

\paragraph{Temporal truncation error.}
For the time discretisation, consider the Crank--Nicolson update applied to \(\partial_t u = F(u)\). Inserting the exact solution and dividing by \(\Delta_t\) gives the one-step consistency defect
\[
    \tau_t^n
    \coloneqq
    \frac{u(t^{n+1}) - u(t^n)}{\Delta_t}
    -
    \frac{1}{2}\Bigl(F(u(t^n)) + F(u(t^{n+1}))\Bigr).
\]
Expanding \(u(t^{n+1})\) and \(F(u(t^{n+1}))\) about \(t^n\) yields
\begin{equation}
    \tau_t^n
    =
    -\frac{\Delta_t^2}{12}u_{ttt}(t^n)
    +
    \mathcal{O}(\Delta_t^4)
    =
    \mathcal{O}(\Delta_t^2).
    \label{eq:ch2-lte-time}
\end{equation}
Thus the Crank--Nicolson time discretisation is second order. The Heun-explicit source update used in the IMEX scheme has the same formal \(\mathcal{O}(\Delta_t^2)\) local order for the explicit source contribution.

\paragraph{Spatial truncation error.}
For the spatial discretisation, the centred second difference satisfies
\[
    \frac{u_{i+1} - 2u_i + u_{i-1}}{\Delta_x^2}
    =
    u_{xx}(x_i)
    +
    \frac{\Delta_x^2}{12}u_{xxxx}(x_i)
    +
    \mathcal{O}(\Delta_x^4).
\]
Consequently, for either diffusion coefficient \(D_j \in \{D_1,D_2\}\), the spatial defect is
\begin{equation}
    \tau_x^n
    =
    \frac{\Delta_x^2}{12}D_j u_{xxxx}(x_i,t^n)
    +
    \mathcal{O}(\Delta_x^4)
    =
    \mathcal{O}(\Delta_x^2).
    \label{eq:ch2-lte-space}
\end{equation}
The present scheme is therefore second order in space, in contrast with the compact finite-difference method of \cite{Zhai_2020}, which attains fourth-order spatial accuracy at the price of a wider and algebraically more involved stencil.

\paragraph{Global error estimate.}
Combining \eqref{eq:ch2-lte-time} and \eqref{eq:ch2-lte-space}, the total consistency defect over one step is \(\mathcal{O}(\Delta_t^2 + \Delta_x^2)\). For the periodic constant-coefficient problem, the Fourier decomposition diagonalises the discrete Laplacian, so each mode evolves under the amplification matrix \(\mathbf{G}(\kappa)\) from \eqref{eq:ch2-amplification-matrix}. Under the stability bound of Theorem~\ref{thm:ch2-cn-stability}, summation over \(N_t = \tau_f/\Delta_t\) steps gives
\[
    \|\mathbf{e}^{N_t}\|_{L^2}
    \le
    C\bigl(\Delta_t^2 + \Delta_x^2\bigr),
\]
with \(C\) independent of the mesh parameters.

\begin{theorem}[Second-order convergence of the linear CN--CN scheme]
    \label{thm:ch2-cn-convergence}
    Let \((u,v)\) be the exact solution of the constant-coefficient pTTM \eqref{eq:ch2-ode-constant-coeff}, and assume \((u,v) \in C^{4,3}(\overline{\Omega}\times[0,\tau_f])\). Let \((u_h,v_h)\) denote the numerical solution produced by the monolithic CN--CN scheme \eqref{eq:ch2-block-1d}. Then, for the periodic problem, the global \(\ell^2\)--\(L^2\) error satisfies
    \[
        \|u-u_h\|_{\ell^2L^2}
        +
        \|v-v_h\|_{\ell^2L^2}
        \le
        C(\tau_f)\bigl(\Delta_t^2 + \Delta_x^2\bigr),
    \]
    where \(C(\tau_f)\) depends on \(\tau_f\) and the physical parameters, but is independent of \(\Delta_t\) and \(\Delta_x\). The same estimate holds for the homogeneous Dirichlet problem after replacing the Fourier basis by the discrete sine basis of the one-dimensional Laplacian.
\end{theorem}
\begin{proof}
    Consistency follows from \eqref{eq:ch2-lte-time} and \eqref{eq:ch2-lte-space}. For the periodic constant-coefficient problem, the Fourier decomposition diagonalises the discrete Laplacian, so each mode evolves under the amplification matrix \(\mathbf{G}(\kappa)\). Theorem~\ref{thm:ch2-cn-stability} gives \(\operatorname{spr}(\mathbf{G}(\kappa))\le 1\) uniformly in \(\kappa\), \(\Delta_t\), and \(\Delta_x\), preventing modal error growth across timesteps. The Lax--Richtmyer equivalence principle then yields the global \(\mathcal{O}(\Delta_t^2 + \Delta_x^2)\) estimate. The argument for homogeneous Dirichlet data is identical after expanding in discrete sine modes rather than Fourier modes.
\end{proof}

\begin{remark}[Convergence of the nonlinear IMEX scheme]
    \label{rem:ch2-imex-convergence}
    For the nonlinear scheme of \S\ref{sec:ch2-nonlinear}, a rigorous convergence proof additionally requires uniform control of the coefficient variations between Picard iterates. Under the constitutive closures \eqref{eq:ch2-dimensionless-coeffs}, the coefficients \(D_1\), \(c_1\), and \(c_2\) are Lipschitz continuous on the temperature rectangle \(\mathcal{R}\) identified in Proposition~\ref{prop:ch2-existence}, so the local truncation order remains \(\mathcal{O}(\Delta_t^2 + \Delta_x^2)\) along Picard iterates that stay within \(\mathcal{R}\). The order conditions for IMEX-RK methods of this type are established in \cite[Theorem~3.1]{Ascher_1997}, while the accuracy preservation of the strong Gauss--Seidel predictor in the partitioned setting is discussed in \cite[Theorem~3.2]{Huang_2019}; together these results formally support the design order of the adopted split. A complete proof, however, would require verifying all hypotheses of those theorems for the specific pTTM closures, which is not pursued here. The empirical convergence study of \S\ref{sec:ch2-convergence-tables} confirms the expected rate for the IMEX variant with implicit diffusion and coupling and explicit source treatment.
\end{remark}

\subsection{Empirical validation}
\label{sec:ch2-convergence-tables}

The analytical results above are complemented by a direct refinement study for the one-dimensional nonlinear pTTM. The error metric is the normalised coupled space--time norm
\[
...
\begin{figure}[htbp]
    ...
    \caption{Convergence study for three IMEX splits of the 1D nonlinear pTTM. Log-log plots of the normalised \(\ell^2\)--\(L^2\) error against \(\Delta_t\) (left) and \(\Delta_x\) (right). The IMEX variant with implicit diffusion and coupling, explicit source (variant 2) achieves the design second order \(p \approx 2\) in both time and space, consistent with Theorem~\ref{thm:ch2-cn-convergence} and Remark~\ref{rem:ch2-imex-convergence}.}
